...
function wholeDateCalc(startDate, endDate) {
    ...
    if(startMonth == 0 && startDay == 1) {
        ...
        if (endMonth == 11 && endDay == 31) {
            ...
            WholeDay = moment(startDate).endOf('month').format("DD") - startDay;
        } else if(endDay == moment(endDate).endOf('month').format("DD")){
            // 非年初非月初且非年末但月末 前取月天后取月中间取年, 如果当年0,跨1年1,跨2年为2;如2018-02-08&&2020-11-30
            wholeYear = endYear > startYear ? endYear - startYear - 1 : endYear - startYear;
            wholeMonth = (12 - 12 - startMonth - 1) + (endMonth + 1);
            WholeDay = moment(startDate).endOf('month').format("DD") - startDay;
        } else {
            // 非年初非月初且非年末非月末 前后都取月天中间取年, 如2018-02-08&&2020-11-3
            wholeYear = endYear > startYear ? endYear - startYear - 1 : endYear - startYear
            wholeMonth = endYear > startYear ? (12 - startMonth) + endMonth : 12 - startMonth;
            WholeDay = moment(startDate).endOf('month').format("DD") - endDay;
        }
    }
    return {wholeYear, wholeMonth, WholeDay}

    // let str = 'wholeYear: ' + wholeYear + '  wholeMonth:' + wholeMonth + '  WholeDay: ' + WholeDay
    // $('#result').text(str);
}